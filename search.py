...
def detect(cfg,
    ...
    for pid in query_pids:
        ...
        print(temp)
        query_feats[pid] = torch.from_numpy(temp).float().to(device)  # torch.Size([2, 2048])
        print(query_feats[pid])
        query_feats[pid] = torch.nn.functional.normalize(query_feats[pid], dim=1, p=2) # 计算出查询图片的特征向量
        print(query_feats[pid])
        ...
        if det is not None and len(det) > 0:
            ...
            if gallery_img:
                ...
                for pid in query_pids:
                    ...
                    distmat = distmat.sum(axis=0) / len(query_feats[pid]) # 平均一下query中同一行人的多个结果
                    index = distmat.argmin()
                    if distmat[index] < dist_thres:
                        print('距离：%s'%distmat[index])
                        plot_one_box(gallery_loc[index], im0, label=str(pid), color=colors[int(cls)])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conf-thres', type=float, default=0.25, help='物体置信度阈值')
