...
class LoadWebcam:  # for inference
    def __init__(self, cam_id='0', img_size=416, half=False):
        self.img_size = img_size
        self.half = half  # half precision fp16 images
        pipe = '0'  # local camera
        if(cam_id=='0'):
            pipe = "rtsp://admin:eksrufthgkr!@192.168.4.80:554/cam/realmonitor?channel=1&subtype=0"
        if(cam_id=='1'):
            pipe = "rtsp://admin:eksrufthgkr!@192.168.4.81:554/cam/realmonitor?channel=1&subtype=0"
        if(cam_id=='2') :
            pipe = "rtsp://admin:eksrufthgkr!@192.168.4.82:554/cam/realmonitor?channel=1&subtype=0"
        print("pipe is "+pipe)
    ...
    def __next__(self):
        ...
        img = np.ascontiguousarray(img, dtype=np.float16 if self.half else np.float32)  # uint8 to fp16/fp32
        img /= 255.0  # 0 - 255 to 0.0 - 1.0
        #cv2.imwrite('./temp/temp.jpg',img0)
